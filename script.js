...
// Vérifie si connecté avant modification
    ...
    ["À faire","En cours","Terminé"].forEach(status=>{
      ...
      (theme[status]||[]).forEach((task,i)=>{
        ...
        let checklistHtml = task.checklist.map(item => 
          `<div class="checklist-item"><input type="checkbox" ${item.done?'checked':''}><span>${item.text}</span></div>`).join("");
        ...
        t.addEventListener("click", ()=>{
          currentEdit={themeId,status,index,task};
...
document.getElementById("modalSave").addEventListener("click", async ()=>{
  ...
  await requireAuth();
  const {themeId,status,index,task} = currentEdit;
